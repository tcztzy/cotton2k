...
//
{
	...
	try
	{
        ...
        {
            BOOL bAdjustToDo;
            tie(bAdjustToDo, date, Daynum, DayEmerge, NumLayersWithRoots, PlantHeight) = DoAdjustments(ProfileName, date, Daynum, DayEmerge, DayStart, DayFinish, DayPlant, NumLayersWithRoots, PlantHeight, RootWeight, RootAge);
            pdlg->ProgressStepit();
            tie(date, Daynum, DayEmerge, NumLayersWithRoots, PlantHeight) = SimulateThisDay(ProfileName, Daynum, DayEmerge, DayStart, DayFinish, DayPlant, NumLayersWithRoots, PlantHeight, RootWeight, RootAge);
            // If there are pending plant adjustments, call WriteStateVariables() to write
            // state variables of this day in a scratch file.
            if (bAdjustToDo)
	            WriteStateVariables(TRUE, date, Daynum, NumLayersWithRoots, PlantHeight, RootWeight, RootAge);
        }
	}
    catch (SimulationEnd){}
...
///////////////////////////////////////////////////////////////////////////////
tuple<BOOL, string, int, int, int, double> C2KApp::DoAdjustments(string ProfileName, const string& Date, const int& daynum, const int& dayEmerge, const int& DayStart, const int& DayFinish, const int& DayPlant, int NumLayersWithRoots, double PlantHeight, double RootWeight[40][20][3], double RootAge[40][20])
...
//  will assign TRUE to nadj(jj) if adjustment is necessary, and compute the necessary parameters.
            ...
            {
                tie(date, Daynum, NumLayersWithRoots, PlantHeight) = PlantAdjustments(i, jj, ProfileName, date, Daynum, NumLayersWithRoots, PlantHeight, RootWeight, RootAge);
...
//////////////////////////////////////////////////
tuple<string, int, int, int, double> C2KApp::SimulateThisDay(string ProfileName, const int& daynum, const int& dayEmerge, const int& DayStart, const int& DayFinish, const int& DayPlant, int NumLayersWithRoots, double PlantHeight, double RootWeight[40][20][3], double RootAge[40][20])
...
//     Call DailyOutput for reporting some simulation data for this day.
      DailyOutput(ProfileName, Date, Daynum, DayEmerge, DayFinish, NumLayersWithRoots, PlantHeight, RootWeight, RootAge);
