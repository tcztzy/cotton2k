...
//
{
    ...
    if (PrdWthFileName.length() > 0) {
        std::string strFileName = "CLIMATE\\" + PrdWthFileName;
    ...
    if (ActWthFileName.length() > 0) {
        std::string strFileName = "CLIMATE\\" + ActWthFileName;
...
//     This function reads the climate data file. It is called by
//     OpenClimateFile(), and it
//  calls GetLineData().
...
//
{
    ...
    //     The following variables inform if the data in the climate file are in
    //     metric or "English" units.
    int iswRad = 0, iswTmp = 0, iswRain = 0, iswDewt = 0, iswWind = 0;
    double AverageWind = 0;
    std::string Name, FileName;
    if (nLength >= 31) iswRad = stoi(Dummy.substr(31, 3));
    if (nLength >= 34) iswTmp = stoi(Dummy.substr(34, 3));
    if (nLength >= 37) iswRain = stoi(Dummy.substr(37, 3));
    if (nLength >= 40) iswWind = stoi(Dummy.substr(40, 3));
    if (nLength >= 43) iswDewt = stoi(Dummy.substr(43, 3));
    if (nLength >= 61) AverageWind = stof(Dummy.substr(61, 10));
    ...
    while (DataFile.eof() == 0) {
        ...
        jdd = stoi(Dummy.substr(0, 4));
        int j = jdd - DayStart;  // days from start of simulation
        //     Daily weather data are stored in the structure Clim, derived from
        //  struct Climstruct {int nDay; float Rad, Tmax, Tmin, Rain, Wind,
        //  Tdew; }
        if (j >= 0) {
            Clim[j].nDay = jdd;
            //     Get float values
            Clim[j].Rad = stof(Dummy.substr(21, 7));  //  Radiation data
            Clim[j].Tmax =
                stof(Dummy.substr(28, 7));  //  maximum temperature data
            Clim[j].Tmin =
                stof(Dummy.substr(35, 7));  //  minimum temperature data
            Clim[j].Rain = stof(Dummy.substr(42, 7));  //  rain data
            //     If iswRad is 1, convert solar radiation from MJ/m2 to
            //     langleys.
            if (iswRad == 1) Clim[j].Rad = Clim[j].Rad * 23.884;
            //     If iswTmp is 0, convert maximum and minimum temperatures from
            //     F to C.
            if (iswTmp == 0) {
            ...
            double f6 = stof(Dummy.substr(49, 7));
            if (iswWind < 0 || f6 <= 0)
                Clim[j].Wind = AverageWind;
            else {
                Clim[j].Wind = f6;
                //     If iswWind is 0, convert daily windrun from miles to km.
                if (iswWind == 0) Clim[j].Wind = Clim[j].Wind * 1.609;
            }
            //     Check if dew point temperature data are available
            if (iswDewt < 0)
                Clim[j].Tdew = -100;
            else {
                Clim[j].Tdew = stof(Dummy.substr(56, 7));
                //     If iswDewt is 0, convert dewpoint temperatures from F to
                //     C.
                if (iswTmp == 0) Clim[j].Tdew = (Clim[j].Tdew - 32) / 1.8;
            }
            //     If iswRain is 0, convert rainfall from inches to mm.
            if (iswRain == 0) Clim[j].Rain = Clim[j].Rain * 25.4;
        }
        //     Estimate dewpoint temperature when it is not available:
        if (Clim[j].Tdew <= -100) Clim[j].Tdew = tdewest(Clim[j].Tmax);
    }
    DataFile.close();
    return jdd;
}
...
//     This function estimates the approximate daily average dewpoint
//     temperature when
...
//  CultivationDate, CultivationDepth, DayFirstDef, DayStartPredIrrig,
//  DayStopPredIrrig, DayWaterTableInput, DefoliationDate, DefoliationMethod,
//  DefoliantAppRate, ElCondSatSoil, Irrig (structure), IrrigationDepth,
//  IrrigMethod, LevelsOfWaterTable, LocationColumnDrip, LocationLayerDrip,
//  MaxIrrigation, MinDaysBetweenIrrig, NFertilizer (structure), NumIrrigations,
//  NumNitApps, NumWaterTableData, pixday, pixmth, pixppa.
//
{
    //     Open the input file
    std::string m_FilePath = "AGINPUT\\" + AgrInputFileName;
    ...
    std::string Dummy = GetLineData(DataFile);
    std::string m_AgrInptDesc;  // Description of the Profile file
    if (Dummy.length() > 20) {
        m_AgrInptDesc = Dummy.substr(20, 55);
        boost::algorithm::trim_right(m_AgrInptDesc);
    } else
        m_AgrInptDesc = "";
    //
    NumNitApps = 0;
    ...
    while (DataFile.eof() == 0) {
        ...
        int isdhrz;  // horizontal placement of DRIP, cm from left edge of soil
                     // slab.
        std::string cdate;  // date of this application.
                            //
        std::string StrTemp = GetLineData(DataFile);
        //     The type of input is defined from the first 5 characters of the
        //     line.
        if (StrTemp.substr(0, 5) == "IRRIG") {
            //     Structure Irrigation {int day, method, LocationColumnDrip,
            //     LocationLayerDrip;
            //                           double amount; }   Irrig[150];
            cdate = StrTemp.substr(19, 11);
            Irrig[NumIrrigations].day =
                DateToDoy(cdate, iyear);  // day of year of this irrigation
            Irrig[NumIrrigations].amount = stof(
                StrTemp.substr(30, 15));  // net amount of water applied, mm
            Irrig[NumIrrigations].method =
                stoi(StrTemp.substr(45, 5));  // method of irrigation: 1= 2=drip
            isdhrz = stoi(StrTemp.substr(50, 5));  // horizontal placement cm
            isddph = stoi(StrTemp.substr(55, 5));  // vertical placement cm
        ...
        //
            ...
            //     Structure NFertilizer {int day; double amtamm, amtnit,
            //     amtura,
            //		                      int mthfrt, ksdr, lsdr; }
            //NFertilizer[150];
            cdate = StrTemp.substr(19, 11);
            NFertilizer[NumNitApps].day =
                DateToDoy(cdate, iyear);  // day of year
            NFertilizer[NumNitApps].amtamm =
                (float)stof(StrTemp.substr(30, 10));  // ammonium N (kg/ha)
            NFertilizer[NumNitApps].amtnit = (float)atof(
                StrTemp.substr(40, 10).c_str());  // nitrate N (kg/ha)
            NFertilizer[NumNitApps].amtura =
                (float)atof(StrTemp.substr(50, 10).c_str());  // urea N (kg/ha)
            NFertilizer[NumNitApps].mthfrt =
                (int)stoi(StrTemp.substr(60, 5));  // method of application
            if (StrTemp.length() >= 70)
                isdhrz = stoi(StrTemp.substr(65, 5));
            else
                isdhrz = 0;
            if (StrTemp.length() >= 75)
                isddph = stoi(StrTemp.substr(70, 5));
            else
                isddph = 0;
            //      If this is a side dressing or drip, convert distances to
            //      soil
            //  layer and column numbers by calling SlabLoc.
            if (NFertilizer[NumNitApps].mthfrt == 1 ||
                NFertilizer[NumNitApps].mthfrt == 3) {
        ...
        //
            ...
            CultivationDepth[icult] = (float)stof(StrTemp.substr(30, 5));
            icult++;
        }
        //
        else if (StrTemp.substr(0, 3) == "DEF") {
            cdate = StrTemp.substr(19, 11);
            int pgrmth = stoi(StrTemp.substr(
                30, 5));  // code number for method of application.
            DefoliationDate[idef] = DateToDoy(cdate, iyear);
            //     If this is input for defoliation prediction, define the rate
            //     as -99.9,
            //  and pgrmth in this case is the percentage of boll opening for
            //  which defoliation will be activated, and cdate is the latest
            //  date for defoliation application.
            ...
            //  Store the method of application code number as DEFMTH(IDEF).
                ...
                double rtepgr =
                    (float)stof(StrTemp.substr(40, 10));  // rate of application
                int pgunit = stoi(StrTemp.substr(50, 5)) -
                             1;  // code number for rate units used.
                if (rtepgr > 0.01) {
                    if (pgunit == 1)
                        rtepgr = rtepgr * 8;
                    else if (pgunit == 2)
        ...
        //
            ...
            cdate = StrTemp.substr(19, 11);
            pixmth[ipx] = stoi(StrTemp.substr(30, 5));
            double rtepgr =
                (float)stof(StrTemp.substr(40, 10));  // rate of application.
            int pgunit = stoi(StrTemp.substr(50, 5)) -
                         1;  // code number for rate units used.
            if (rtepgr > 0.01) {
                ...
                if (pgunit == 1) rtepgr = rtepgr * 8;
                if (pgunit == 2) rtepgr = rtepgr / 16;
                if (pgunit == 4) rtepgr = 1 / rtepgr;
                if (pgunit == 5) rtepgr = 8 / rtepgr;
                pixppa[ipx] = rtepgr;
            }
            ipx++;
        }
        //
            ...
            cdate = StrTemp.substr(19, 11);
            LevelsOfWaterTable[NumWaterTableData] =
                (float)stof(StrTemp.substr(30, 5));
            ElCondSatSoil[NumWaterTableData] =
                (float)stof(StrTemp.substr(35, 10));
            DayWaterTableInput[NumWaterTableData] = DateToDoy(cdate, iyear);
            NumWaterTableData++;
        }
        //
        else if (StrTemp.substr(0, 5) == "PREDI") {
            MaxIrrigation = stof(StrTemp.substr(20, 7));
            IrrigMethod = stoi(StrTemp.substr(28, 2));
            std::string datstrir = StrTemp.substr(30, 15);
            boost::algorithm::erase_all(datstrir, " ");
            DayStartPredIrrig = DateToDoy(
                datstrir, iyear);  // date to start the predicted irrigation.
            std::string datstpir = StrTemp.substr(45, 15);
            boost::algorithm::erase_all(datstpir, " ");
            DayStopPredIrrig = DateToDoy(
                datstpir, iyear);  // date to stop the predicted irrigation.
            MinDaysBetweenIrrig = stoi(StrTemp.substr(60, 5));
            isdhrz = stoi(StrTemp.substr(65, 5));
            isddph = stoi(StrTemp.substr(70, 5));
            IrrigationDepth = (float)stof(StrTemp.substr(75, 5));
    ...
    //     Write irrigations set by input:
    {
        ...
        File20 << "       Date           Amount        Method          Drip "
                  "Location "
               << endl;
        if (OutIndex[1] == 0)
            File20 << "                        mm                        "
        ...
        for (int ii = 0; ii < NumIrrigations; ii++) {
            File20.width(14);
            File20 << DoyToDate(Irrig[ii].day,
                                iyear);  // date of this irrigation
            ...
            if (OutIndex[1] == 0)
                ...
                File20 << Irrig[ii].amount /
                              25.4;  // net amount of water applied, inches
            //     Methods of irrigation: (0, "SPRINKLER") (1, "FURROW") (2,
            //     "DRIP")
            std::string IrrMethod = "";
            ...
            if (Irrig[ii].method == 2) {
                File20.width(12);
                File20
                    << Irrig[ii].LocationColumnDrip;  // horizontal placement cm
                File20.width(12);
    ...
    {
        File20 << endl
               << "    Nitrogen applications set by input file: " << endl;
        File20 << "       Date           NH4 N        NO3 N       Urea N       "
                  " Method       Drip Location "
               << endl;
        if (OutIndex[1] == 0)
            File20 << "                                  kg / ha               "
                      "                Column       Layer "
        ...
        for (int ii = 0; ii < NumIrrigations; ii++) {
            File20.width(14);
            File20 << DoyToDate(NFertilizer[ii].day,
                                iyear);  // date of this irrigation
            File20.setf(ios::fixed);
            File20.width(13);
            File20.precision(2);
            if (OutIndex[1] == 0) {
                ...
                File20.width(13);
                File20
                    << NFertilizer[ii].amtura;  // net amount of Urea N, kg/ha
            } else {
                File20 << NFertilizer[ii].amtamm /
                              1.121F;  // net amount of NH4 N, lbs/ac
                File20.width(13);
                File20 << NFertilizer[ii].amtnit /
                              1.121F;  // net amount of NO3 N, lbs/ac
                File20.width(13);
                File20 << NFertilizer[ii].amtura /
                              1.121F;  // net amount of Urea N, lbs/ac
            }
            //     Methods of application: (0, "Broadcast") (1, "Side-dress")
            //     (2, "Foliar") (3, "DRIP")
            std::string FMethod = "";
    ...
    //     Write cultivations set by input:
    {
        ...
        for (int ii = 0; ii < icult; ii++) {
            File20.width(14);
            File20 << DoyToDate(CultivationDate[ii],
                                iyear);  // date of this irrigation
            File20.setf(ios::fixed);
            File20.width(14);
            File20.precision(2);
    ...
    {
        File20 << endl
               << "    Defoliation applications set by input file: " << endl;
        File20 << "       Date           Method          Rate" << endl;
        for (int ii = 0; ii < idef; ii++) {
            if (DefoliantAppRate[ii] == -99.9) continue;
            File20.width(14);
            File20 << DoyToDate(DefoliationDate[ii],
                                iyear);  // date of this application
            File20.setf(ios::fixed);
            File20.width(14);
        ...
        for (int ii = 0; ii < idef; ii++) {
            if (DefoliantAppRate[ii] == -99.9) {
                ...
                File20 << endl
                       << " Setting prediction of defoliation at percentage "
                          "boll opening of ";
                File20.width(10);
                File20 << DefoliationMethod[ii]
                       << endl;  // boll opening percentage
                File20 << "         Last date of defoliation prediction:  ";
                File20 << DoyToDate(DefoliationDate[ii], iyear)
                       << endl;  // last date of prediction
            }
        }
    }
    ...
    {
        File20 << endl
               << "    Water Table and Salinity Data set by input file: "
               << endl;
        File20 << "       Date       Water Table, cm   Electr-Conductivity"
               << endl;
        for (int ii = 0; ii < NumWaterTableData; ii++) {
            File20.width(14);
            File20 << DoyToDate(DayWaterTableInput[ii],
                                iyear);  // date of this application
            File20.setf(ios::fixed);
            File20.width(14);
    ...
    //     Write data for setting predicted irrigations:
    {
        ...
        File20 << " Stopping Date " << DoyToDate(DayStopPredIrrig, iyear)
               << endl;
...
//     This function computes the layer (lsdr) or column (ksdr) where the
//     emitter
//  of drip irrigation, or the fertilizer side - dressing is located. It is
//  called from ReadAgriculturalInput().
