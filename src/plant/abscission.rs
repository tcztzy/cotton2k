...
unsafe fn pre_fruit_leaf_abscission(droplf: f64) {
...
unsafe fn main_stem_leaf_abscission(k: usize, l: usize, droplf: f64) {
...
unsafe fn fruit_node_leaf_abscission(k: usize, l: usize, m: usize, droplf: f64) {
...
unsafe fn defoliation_leaf_abscission() {
...
pub unsafe fn fruiting_sites_abscission() {
...
unsafe fn site_abscission_ratio(k: usize, l: usize, m: usize, lt: usize) -> f64 {
    ...
    if FruitingCode[k][l][m] == 1 {
        if AgeOfSite[k][l][m] < VABSC[3] {
            ...
            if xsqage >= VABSC[0] {
                ...
                pabs = VarPar[46]
                    + (VarPar[45] - VarPar[46]) * ((VABSC[0] - xsqage) / VABSC[0]).powf(VABSC[1]);
            }
        }
        shedt = 1.0 - (1.0 - ShedByCarbonStress[lt]) * (1.0 - ShedByNitrogenStress[lt]);
    } else if FruitingCode[k][l][m] == 7 && AgeOfBoll[k][l][m] <= VarPar[47] {
        pabs = VarPar[48];
        shedt = 1.0 - (1.0 - ShedByCarbonStress[lt]) * (1.0 - VABSC[2] * ShedByNitrogenStress[lt]);
    } else if AgeOfBoll[k][l][m] > VarPar[47] && AgeOfBoll[k][l][m] <= (VarPar[47] + VarPar[49]) {
        pabs =
            VarPar[48] - (VarPar[48] - VarPar[50]) * (AgeOfBoll[k][l][m] - VarPar[47]) / VarPar[49];
        shedt = 1.0
...
unsafe fn square_abscission(k: usize, l: usize, m: usize, abscission_ratio: f64) {
...
unsafe fn boll_abscission(k: usize, l: usize, m: usize, abscission_ratio: f64, gin1: f64) {
...
unsafe fn adjust_square_abscission(k: usize, l: usize, m: usize, abscsq: f64) {
...
unsafe fn adjust_young_boll_abscission(k: usize, l: usize, m: usize, abscgb: f64, gin1: f64) {
...
unsafe fn adjust_boll_abscission(k: usize, l: usize, m: usize, jx: i32, gin1: f64) {
    if FruitFraction[k][l][m] <= 0.001 {
        SeedNitrogen -= BollWeight[k][l][m] * (1.0 - gin1) * SeedNConc;
        BurrNitrogen -= BurrWeight[k][l][m] * BurrNConc;
        CumPlantNLoss += BollWeight[k][l][m] * (1.0 - gin1) * SeedNConc;
...
unsafe fn compute_site_numbers() {
    NumSquares = 0.0;
    NumGreenBolls = 0.0;
    NumOpenBolls = 0.0;
    for k in 0..NumVegBranches as usize {
...
#[inline]
unsafe fn NodeLayerPreFru(index: usize) -> usize {
    crate::NodeLayerPreFru[index] as usize
}
