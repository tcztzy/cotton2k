...
impl Serialize for State {
        ...
        S: Serializer,
    {
        ...
        state.serialize_field("plant_height", &self.plant_height)?;
        state.serialize_field("plant_weight", &self.plant_weight)?;
        state.serialize_field("lint_yield", &self.lint_yield)?;
        state.end()
    }
}
...
impl Serialize for Simulation {
    ...
    {
        let states = unsafe {
            std::slice::from_raw_parts(self.states, (self.day_finish - self.day_start + 1) as usize)
        };
        let day_start = NaiveDate::from_yo(self.year, self.day_start);
        let day_finish = NaiveDate::from_yo(self.year, self.day_finish);
        let day_emerge = NaiveDate::from_yo(self.year, self.day_emerge);
        let mut simulation = serializer.serialize_struct("Simulation", 3)?;
        simulation.serialize_field("start_date", &day_start.format("%Y-%m-%d").to_string())?;
        simulation.serialize_field("end_date", &day_finish.format("%Y-%m-%d").to_string())?;
        simulation.serialize_field("emerge_date", &day_emerge.format("%Y-%m-%d").to_string())?;
        simulation.serialize_field("states", &states)?;
        simulation.end()
    }
}

#[no_mangle]
extern "C" fn output_json(sim: &Simulation) {
    let profile_name = unsafe { CStr::from_ptr(sim.profile_name).to_str().unwrap() };
    let mut file = std::fs::File::create(format!("output/{}.json", profile_name)).expect("create failed");
    let content = serde_json::to_string(&sim).unwrap();
