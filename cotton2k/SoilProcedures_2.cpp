...
//     This function computes the water redistribution in the soil after
//     irrigation
//  by a drip system. It also computes the resulting redistribution of nitrate
//  and urea N. It is called by SoilProcedures() noitr times per day. It calls
//  function CellDistrib().
...
//
{
    ...
    double dripn[40];  // amount of nitrate N applied, or going from one ring of
                       // soil soil cells to the next one, mg. (array)
    double dripu[40];  // amount of urea N applied, or going from one ring of
                       // soil soil cells to the next one, mg. (array)
    for (int i = 0; i < 40; i++) {
        dripw[i] = 0;
        dripn[i] = 0;
    ...
    //     Incoming flow of water (Drip, in mm) is converted to dripw(0), in cm3
    //     per slab.
    dripw[0] = Drip * RowSpace * .10;
    //     Wetting the cell in which the emitter is located.
    double h2odef;  // the difference between the maximum water capacity (at a
    ...
    if (VolNo3NContent[l0][k0] > 1.e-30) {
        ...
        if ((cnw * MaxWaterCapacity[l0]) <
            (NO3FlowFraction[l0] * VolNo3NContent[l0][k0])) {
            dripn[1] =
                NO3FlowFraction[l0] * VolNo3NContent[l0][k0] * dl[l0] * wk[k0];
            VolNo3NContent[l0][k0] =
                (1 - NO3FlowFraction[l0]) * VolNo3NContent[l0][k0];
        } else {
            dripn[1] = dripw[1] * cnw;
    ...
    if (VolUreaNContent[l0][k0] > 1.e-30) {
        cuw = VolUreaNContent[l0][k0] /
              (VolWaterContent[l0][k0] + dripw[0] / (dl[l0] * wk[k0]));
        if ((cuw * MaxWaterCapacity[l0]) <
            (NO3FlowFraction[l0] * VolUreaNContent[l0][k0])) {
            dripu[1] =
                NO3FlowFraction[l0] * VolUreaNContent[l0][k0] * dl[l0] * wk[k0];
            VolUreaNContent[l0][k0] =
                (1 - NO3FlowFraction[l0]) * VolUreaNContent[l0][k0];
        } else {
            dripu[1] = dripw[1] * cuw;
            VolUreaNContent[l0][k0] = MaxWaterCapacity[l0] * cuw;
        }
    }
    double defcit[40][20];  // array of the difference between water capacity
    ...
    for (int kr = 1; kr < maxl; kr++) {
        ...
        double sn1 =
            0;  // sum of movable nitrate N content in a ring of cells, mg
        ...
        for (int l = 1; l < nl; l++) {
            ...
            for (int k = 0; k < nk; k++) {
                ...
                if (dist <= radius && dist > (radius - 6)) {
                    ...
                    sn1 += VolNo3NContent[l][k] * dl[l] * wk[k] *
                           NO3FlowFraction[l];
                    su += VolUreaNContent[l][k] * dl[l] * wk[k];
                    su1 += VolUreaNContent[l][k] * dl[l] * wk[k] *
                           NO3FlowFraction[l];
        ...
        if (dripw[kr] <= h2odef) {
            ...
            for (int l = 1; l < nl; l++) {
                for (int k = 0; k < nk; k++) {
                    ...
                    if (dist <= radius && dist > (radius - 6)) {
                        VolWaterContent[l][k] +=
                            dripw[kr] * defcit[l][k] / h2odef;
                        VolNo3NContent[l][k] +=
                            dripn[kr] * defcit[l][k] / h2odef;
                        VolUreaNContent[l][k] +=
                            dripu[kr] * defcit[l][k] / h2odef;
                    }
                }  // end loop k
            }      // end loop l
            return;
        ...
        double drwout =
            dripw[kr] -
            h2odef;  //  the amount of water going out of a ring, cm3.
        ...
        double drnout =
            drwout * cnw;   //  the amount of nitrate N going out of a ring, mg
        ...
        double druout =
            drwout * cuw;   //  the amount of urea N going out of a ring, mg
...
//  It is called from DripFlow().
{
    ...
    if (l > l0) {
        for (int il = l0; il <= l; il++) xl += dl[il];
        xl -= (dl[l] + dl[l0]) * 0.5;
    } else if (l < l0) {
        for (int il = l0; il >= l; il--) xl += dl[il];
        xl -= (dl[l] + dl[l0]) * 0.5;
    ...
    if (k > k0) {
        for (int ik = k0; ik <= k; ik++) xk += wk[ik];
        xk -= (wk[k] + wk[k0]) * 0.5;
    } else if (k < k0) {
        for (int ik = k0; ik >= k; ik--) xk += wk[ik];
        xk -= (wk[k] + wk[k0]) * 0.5;
