...
//     This function adjusts plant height and plant fruiting map, when data for
//     such
...
//  MapDataPlantHeight, MapDataSquareNum, NumAdjustDays, NumFruitBranches,
//  NumFruitSites, NumGreenBolls, NumOpenBolls, NumPreFruNodes, NumSquares,
//  PlantHeight.
...
//  jj - the type of this adjustment.
{
    ...
    switch (jj) {
        case 0:  // adjust the number of main stem nodes
            if (MapDataMainStemNodes[i] <= 0) return;
            //     Compute targeted number of adjusted number of fruiting
            //     branches (mntarget).
            //  The difference between the measured and the simulated number of
            //  nodes should be more than one node. Otherwise no adjustments are
            //  made.
            ...
            //     Compute the ratio used to adjust the rate of formation of
            //     mainstem nodes.
            int mn0;  // simulated number of main stem nodes, at the start of
            ...
            if (mnsim != mn0) {
                ...
                if (AdjAddMSNodesRate > 0.98 && AdjAddMSNodesRate < 1.02)
                    AdjAddMSNodesRate = 1;
                if (AdjAddMSNodesRate < 0) AdjAddMSNodesRate = 0;
            }
            //
        ...
        case 1:  // Plant stem height
            if (MapDataPlantHeight[i] <= 0) return;
            //     Compute targeted adjusted plant height, if difference is
            //     larger than
            //  5% of the average.  Note that adjustment is by 90% of the
            //  difference.
            double zsim;  //   simulated plant height.
            zsim = PlantHeight;
            double ddif;  // the difference between simulated and plant
                          // adjustment values.
            ...
            //     The difference should be at least 5% of the height, otherwise
            //     no adjustments made.
            ...
            AdjAddHeightRate = 1;
            pHeight =
                Scratch21[DayOfSimulation - NumAdjustDays - 1].plantHeight;
            if (fabs(zsim - pHeight) > 0)
                AdjAddHeightRate = (ztarget - pHeight) / (zsim - pHeight);
            if (AdjAddHeightRate <
                0)  //  no negative growth rates for plant height.
                AdjAddHeightRate = 0;
        ...
        case 2:  // total number of fruiting sites
            if (MapDataAllSiteNum[i] <= 0) return;
            //     If first square has not yet been simulated, but map data
            //     shows squares,
            ...
            //     Compute the ratio to adjust the rate of formation of sites on
            //     fruiting branches.
            if (sitarget <= 0) AdjAddSitesRate = 1;
            int pNumFruitSites;  // number of sites at start of adjustment
            pNumFruitSites =
                Scratch21[DayOfSimulation - NumAdjustDays - 1].numFruitSites;
            if (sitesim != pNumFruitSites) {
                AdjAddSitesRate =
                    (sitarget - pNumFruitSites) / (sitesim - pNumFruitSites);
                if (AdjAddSitesRate > 0.98 && AdjAddSitesRate < 1.02)
                    AdjAddSitesRate = 1;
                if (AdjAddSitesRate < 0) AdjAddSitesRate = 0;
            }
        ...
        case 3:  // number of squares
            if (MapDataSquareNum[i] <= 0) return;
            //     Adjust actual square numbers. Amount to add each day is the
            //  difference between target and actual simulated value on
            //  adjustment date divided by the length of the  adjustment period.
        ...
        case 4:  // number of green bolls
            if (MapDataGreenBollNum[i] <= 0) return;
            //     Adjust actual green boll numbers using the same method as for
            //     squares.
            if ((NumGreenBolls - MapDataGreenBollNum[i]) > 1 &&
                NumAdjustDays > 0) {
                AdjGreenBollAbsc =
                    1 - pow((MapDataGreenBollNum[i] / NumGreenBolls),
                            (1 / (double)NumAdjustDays));
                nadj[4] = true;
