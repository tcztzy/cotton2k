...
//     ProfileName The following argument is used:
...
//
{
    ...
    if (OutIndex[2] == 0)  // per plant
    {
        if (OutIndex[1] == 0)  // metric units
        {
            ...
            File46 << "                  ------------------  "
                      "-----------------------------------"
                   << endl;
            File46 << " DAE     Date     Height  LAI  Nodes  Sites  Squares  "
                      "Green  Open   Absc  Yield"
                   << endl;
            File46 << "                   (cm)                               "
                      "bolls  bolls  sites kg/ha"
                   << endl;
        } else  // English units
        {
            File46 << "                       Per plant                      "
    ...
    File22 << endl
           << "                         SUMMARY OF SIMULATION RUN" << endl;
    ...
    File22 << "                                   PLANT                   "
              "GREEN  OPEN"
           << endl;
    File22 << "     EVENT         DATE       DAE  HEIGHT NODES  LAI  SQARS "
              "BOLLS  BOLLS YIELD"
           << endl;
    ...
    imap =
        OutIndex[8] + OutIndex[9] + OutIndex[10] + OutIndex[11] + OutIndex[12];
    if (imap > 0) ofstream File23("Output\\" + ProfileName + ".SMP", ios::out);
    //
    ...
    if (imap > 0) ofstream File24("Output\\" + ProfileName + ".PLM", ios::out);
    //
    //      The following files are used for checking during model
    // development. They are not intended for general use.
    //      For the soil water data output, two files are opened. The extensions
    //      are '.WAT'
    //  and ".WA2". These files are opened when the flag OutIndex[17] is
    //  non-zero.
    if (OutIndex[17] > 0) {
    ...
    //
    //      When the output flag 15 is non-zero: file '.TM1' which is used for
    //      output
    //  of computed 24-hour weather data for plotting is opened, and its header
    //  line is written.
    //      When the output flag 15 is greater than 1, file '.TM2' which is used
    //      for detailed output
    ...
    //
    //      When the output flag 18 is non-zero, file '.CHB', which outputs
    //      plant carbon balance data,
    //  is opened, and the heading for this file is written.
    if (OutIndex[18] > 0) {
        ofstream File36("Output\\" + ProfileName + ".CHB", ios::out);
        File36
            << "    DATE       CDSTEM   CDLEAF     CDPET    CDROOT    CSTRES   "
               " STEMWT    LEAFWT     PETWT    ROOTWT    SUMPDR      NR";
        File36 << endl;
    }
    //
    //      When the output flag 19 is non-zero, file '.LWP', which outputs
    //      simulated leaf water
    //  potential and related data, is opened, and the heading for this file is
    //  written.
    if (OutIndex[19] > 0) {
        ofstream File44("Output\\" + ProfileName + ".LWP", ios::out);
        File44 << " KDAY  PSISOIL   COND      RSOIL     RROOT     RSHOOT    "
                  "RLEAF     PSILN     PSILD ";
        File44 << endl;
    }
    //
    //      When the output flag 20 is non-zero, files '.NB0' to 'NB4', which
    //      output nitrogen data,
    ...
    if (OutIndex[20] > 0) {
        ...
        File35 << endl;
        File35 << "       0 - 30  30 - 60  60 - 90 90 - 120  0 - 30  30 - 60  "
                  "60 - 90 90 - 120";
        File35 << endl;
        File35 << "                   in parts per million (per volume)"
               << endl;
        ofstream File47("Output\\" + ProfileName + ".NB1", ios::out);
        File47 << " KDAY  BALSN    FERN     ORGN   UPTAKEN    SOILN   SNLOSS   "
                  " BALPN    ADDN   PLANTN    NLOSS";
        File47 << endl;
        ofstream File37("Output\\" + ProfileName + ".NB2", ios::out);
        File37 << "    CHECKING  NITROGEN IN PLANT PARTS  ";
        File37 << endl;
        File37 << " KDAY SLEAFN   PETN  STEMN  ROOTN  BURRN  SEEDN SQRN   "
                  "LEAFCN  PETCN STEMCN ROOTCN  BURCN SEEDCN PETCNO3";
        File37 << endl;
        ofstream File38("Output\\" + ProfileName + ".NB3", ios::out);
        File38 << "    CHECKING  NITROGEN REQUIREMENTS";
        File38 << endl;
        File38 << " KDAY RQNLEF RQNPET RQNSTM RQNRUT   REQV RQNSQR RQNSED "
                  "RQNBUR   REQF   REQTOT";
        File38 << endl;
        ofstream File39("Output\\" + ProfileName + ".NB4", ios::out);
        File39 << "    CHECKING  NITROGEN SUPPLY POOLS";
        File39 << endl;
        File39 << " KDAY SUPNO3 SUPNH4   UPTN LEAFRS PETRS  STEMRS ROOTRS "
                  "BURRES   RESN   PETN  NPOOL";
        File39 << endl;
    }
}
////////////////////////////////////////////////////////////////////////////
void DailyOutput()
//     DailyOutput() writes output at the end of each day. It is called from
//     SimulateThisDay().
//  This function calls WriteStateVariables(), cotplt(), and output1().
//
//     The following global variables are referenced here:
//       Daynum, DayOfSimulation, DayStartPlantMaps, DayStopPlantMaps, dl,
//       NumFruitBranches, NumPreFruNodes, OutIndex, PlantMapFreq, RowSpace,
//       VolNo3NContent, wk,
//     The following global variables are set here:      MainStemNodes,
//     SumNO3N90.
{
    //     1. Compute some variables needed for output:
    //     Main stem node count (MainStemNodes) starts from the cotyledonary
    //     node ( # = 0 ).
    //  After first square, last prefruiting node becomes first fruiting branch
    //  node.
    if (NumFruitBranches[0] <= 0)
        MainStemNodes = NumPreFruNodes - 1;
    else
        MainStemNodes = NumPreFruNodes + NumFruitBranches[0] - 2;
    //     Compute SumNO3N90 as the total nitrate N, up to a depth of 90 cm of
    //     soil, in kg / ha
    SumNO3N90 = 0;
    ...
    //
    //     2. Call WriteStateVariables() which saves values of all important
    //     state variables for
    //  this day in structure Scratch21, which will be used for output at the
    //  end of the simulation.
    if (DayOfSimulation > 0) WriteStateVariables(false);
    //
    //     3. Write plant map output by calling cotplt():
    //     If the output flags (OutIndex(7) or OutIndex(13)) indicate that plant
    //     map output is
    //  requested, and if it is after first square - call function cotplt() if
    //  day of year is between DayStartPlantMaps and DayStopPlantMaps.
    if (FruitingCode[0][0][0] > 0 && (OutIndex[7] + OutIndex[13]) > 0)
        if (Daynum >= DayStartPlantMaps && Daynum <= DayStopPlantMaps) {
            ...
            if ((idum % PlantMapFreq) == 0) {
                if (OutIndex[7] > 0) cotplt(1);
                if (OutIndex[13] > 0) cotplt(4);
            }
        }
    //
    ...
    if (Daynum >= DayFinish || LeafAreaIndex < 0.0002 ||
        Daynum >= LastDayWeatherData)
        bEnd = true;
}
//////////////////////////
void output1()
//     This function is a collection of write statements for output of model
//     results.
//  It writes daily data to files F01 and S01. It is called each day from
//  DailyOutput().
//
//     The following global variables are referenced here:
//       AbscisedFruitSites, Date, Daynum, DayEmerge, FirstBloom, FirstSquare,
//       isw, Kday, LeafAreaIndex, LastDayWeatherData, LintYield, MainStemNodes,
//       NumFruitSites, NumGreenBolls, NumOpenBolls, NumSquares, OutIndex,
//       PlantHeight, PlantPopulation, ProfileName
{
    ...
    if (Daynum >= DayEmerge && isw > 0) {
        //     Convertion for site numbers etc. to per area basis (metric or
        //     English units)
        ...
        //     Write Kday, Date, plant height, LAI, main stem nodes, numbers
        //     (per plant, or converted
        //  to per m2, or to 1000s per acre) of fruiting sites, squares, green
        //  bolls, open bolls, and cumulated abscised sites, and LintYield (in
        //  kg per ha, or lbs per acre).
        File46.unsetf(ios::left);
        File46.width(4);
        File46 << Kday;
        File46.width(12);
        ...
        //  the day of first square, first bloom, or day of last actual weather
        //  data.
...
/////////////////////////////////////////////////////////////////////////////////////
void DataOutput()
//     This function is called from RunSimulation() at the end of the
//     simulation. It
//  gets the data from structure Scratch21 and writes summary data in file
//  *.S01.
//     It calls the functions WriteLine22(), outputplt(), output2(), output3(),
//     output4(),
...
//
//     The following global variables are set here (extracted from the structure
//     Scratch21):
...
//
{
    ...
    double sixpct =
        NumOpenBolls * 0.6;  // 60 percent of the final number of open bolls.
    ofstream File22("Output\\" + ProfileName + ".S01", ios::app);
    ...
    for (int irec = 0; irec < 400; irec++) {
        if (Scratch21[irec].daynum <= 0) break;
    ...
    outputplt();
    if (OutIndex[6] > 0) output2();
    if (OutIndex[3] > 0) output3();
    if (OutIndex[4] > 0) output4();
    if (OutIndex[5] > 0) output5();
    output6();
    if (OutIndex[8] + OutIndex[9] + OutIndex[10] + OutIndex[11] + OutIndex[12] >
        0)
        output7();
}
////////////////////////
void WriteLine22(ofstream &File22, double i00, double i01, double i02)
//     This function writes a formatted line in file *.S01. It is called from
//     DataOutput(). Global variables referenced:
