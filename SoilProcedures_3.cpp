...
{
    //     Compute the modified light interception factor (LightInter1) for use
    //     in computing
    //  transpiration rate.
    double LightInter1;  // modified light interception factor by canopy
    LightInter1 = LightIntercept * 1.55 - 0.32;
    if (LightInter1 < LightIntercept) LightInter1 = LightIntercept;
    if (LightInter1 > 1) LightInter1 = 1;
    ...
    double uptk[40]
               [20];  // actual transpiration from each soil cell, cm3 per day
    for (int l = 0; l < nl; l++)
        for (int k = 0; k < nk; k++) {
            upf[l][k] = 0;
            uptk[l][k] = 0;
        }
    double sumep = 0;  // sum of actual transpiration from all soil soil cells,
                       // cm3 per day.
    //     Compute the reduction due to soil moisture supply by function
    //     PsiOnTranspiration().
    double Transp;  //	the actual transpiration converted to cm3 per slab
                    //units.
    Transp = .10 * RowSpace * PotentialTranspiration *
             PsiOnTranspiration(AverageSoilPsi);
    double difupt =
        0;  // the cumulative difference between computed transpiration
    ...
    do {
        ...
        for (int l = 0; l < NumLayersWithRoots; l++) {
            ...
            //     Compute, for each layer, the lower and upper water content
            //     limits for the transpiration
            //  function. These are set from limiting soil water potentials (-15
            //  to -1 bars).
            double vh2lo;  //  lower limit of water content for the
                           //  transpiration function
            double vh2hi;  //  upper limit of water content for the
                           //  transpiration function
            ...
            for (int k = RootColNumLeft[l]; k <= RootColNumRight[l]; k++) {
                double
                    redfac;  // reduction factor for water uptake, caused by low
                             // levels of soil water, as a linear function of
                             // VolWaterContent, between vh2lo and vh2hi.
                redfac = (VolWaterContent[l][k] - vh2lo) / (vh2hi - vh2lo);
                if (redfac < 0) redfac = 0;
                if (redfac > 1) redfac = 1;
                //     The computed 'uptake factor' (upf) for each soil cell is
                //     the
                //  product of 'root weight capable of uptake' and redfac.
                upf[l][k] = RootWtCapblUptake[l][k] * redfac;
                supf += upf[l][k];
            }  // end loop k
        }      // end loop l
               //
        ...
        for (int l = 0; l < NumLayersWithRoots; l++)
            for (int k = RootColNumLeft[l]; k <= RootColNumRight[l]; k++)
                if (upf[l][k] > 0 && VolWaterContent[l][k] > thetar[l]) {
                    //     The amount of water extracted from each cell is
                    //     proportional to its 'uptake factor'.
                    double
                        upth2o;  // transpiration from a soil cell, cm3 per day
                    upth2o = Transp * upf[l][k] / supf;
                    //     Update VolWaterContent cell, storing its previous
                    //     value as vh2ocx.
                    double vh2ocx;  // previous value of VolWaterContent of this
                                    // cell
                    vh2ocx = VolWaterContent[l][k];
                    VolWaterContent[l][k] -= upth2o / (dl[l] * wk[k]);
                    //     If the new value of VolWaterContent is less than the
                    //     permanent wilting point, modify the
                    //  value of upth2o so that VolWaterContent will be equal to
                    //  it.
                    if (VolWaterContent[l][k] < thetar[l]) {
                        VolWaterContent[l][k] = thetar[l];
                        double xupt;  // intermediate computation of upth2o
                        //     Compute the difference due to this correction and
                        //     add it to difupt.
                        xupt = (vh2ocx - thetar[l]) * dl[l] * wk[k];
                        difupt += upth2o - xupt;
                        upth2o = xupt;
                    }
                    if (upth2o < 0) upth2o = 0;
                    //     Compute sumep as the sum of the actual amount of
                    //     water extracted from all
                    //  soil cells. Recalculate uptk of this soil cell as
                    //  cumulative upth2o.
                    sumep += upth2o;
                    uptk[l][k] += upth2o;
                }  // end of loops l & k, and if upf
        //     If difupt is greater than zero, redefine the variable Transp as
        //     difuptfor use in next loop.
        if (difupt > 0) Transp = difupt;
    } while (difupt > 0);  // end of do; repeat if difupt greater than 0.
    //  recompute SoilPsi for all soil cells with roots by calling function
    //  PSIQ,
    for (int l = 0; l < NumLayersWithRoots; l++) {
        int j = SoilHorizonNum[l];
        for (int k = RootColNumLeft[l]; k <= RootColNumRight[l]; k++)
    ...
    SupplyNH4N = 0;
    SupplyNO3N = 0;
    //     Compute the proportional N requirement from each soil cell with
    //     roots,
    ...
    if (sumep > 0 && TotalRequiredN > 0) {
        for (int l = 0; l < NumLayersWithRoots; l++)
            for (int k = RootColNumLeft[l]; k <= RootColNumRight[l]; k++) {
                if (uptk[l][k] > 0) {
                    double reqnc;  // proportional allocation of TotalRequiredN
                                   // to each cell
