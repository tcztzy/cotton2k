...
//     This function simulates the potential growth of fruiting sites of cotton
//     plants.
...
//        PotGroBurrs, PotGroSquares.
{
    ...
    // night lengths.
    double tfrt;  // the effect of temperature on rate of boll, burr or square
                  // growth.
    ...
    //     cultivar - specific.
    double agemax =
        VarPar[9];  // maximum boll growth period (physiological days).
    double rbmax = VarPar[10];  // maximum rate of boll (seed and lint) growth,
                                // g per boll per physiological day.
    double wbmax = VarPar[11];  // maximum possible boll (seed and lint) weight,
                                // g per boll.
                                //     Loop for all vegetative stems.
    for (int k = 0; k < NumVegBranches; k++)  // loop of vegetative stems
    {
        ...
        for (int l = 0; l < nbrch; l++)  // loop of fruiting branches
        {
            ...
            for (int m = 0; m < nnid;
                 m++)  // loop for nodes on a fruiting branch
            {
                ...
                //            ratebol = 4 * rbmax * pex / (1. + pex)**2
                    ...
                    //     growth.
                    double pex =
                        exp(-4 * rbmax * (AgeOfBoll[k][l][m] - agemax) / wbmax);
