//! Soil temperature initialization and daily thermal-balance simulation.
//!
//! [`SoilThermodynamics`] owns numerical scratch arrays for layer depths,
//! temperatures, heat capacities, and radiation-column factors. [`simulate`]
//! consumes the initialized [`Profile`] and synchronized legacy state, then
//! writes soil, canopy, and mulch temperatures back for downstream soil and
//! plant phases. It may return [`Cotton2KError`] when a thermal iteration cannot
//! converge; it performs no file I/O.
...
/// Thermal scratch state for soil, canopy, and mulch temperature simulation.
...
impl SoilThermodynamics {
    /// Initializes layer temperatures and thermal coefficients from a profile.
    ...
    /// Advances one daily soil, canopy, and mulch temperature calculation.
    ///
    /// The method is called from the root engine after atmospheric state and
    /// canopy shading are available. It mutates the synchronized legacy thermal
    /// fields and may stop early when the global simulation-end flag is set.
