//! Leaf and fruiting-site abscission routines for the plant sub-model.
//!
//! The public routines read the synchronized legacy plant state, apply age,
//! stress, defoliation, and fruit-retention rules, then write the updated state
//! back through the shared compatibility boundary. They mutate global model
//! state, do not perform I/O, and must run inside the serialized simulation.
...
/// Removes leaves whose age or defoliation treatment exceeds retention limits.
...
/// Applies carbon, nitrogen, and water stress to fruiting-site retention.
