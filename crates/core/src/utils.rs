//! Geometry, bounded numeric, and environmental helper functions.
//!
//! The helpers convert physical distances to model-grid locations, clamp legacy
//! scalar calculations, and provide the ambient CO₂ correction. Grid-location
//! routines return [`Cotton2KError`] for coordinates outside the configured
//! slab; the remaining helpers are pure and panic-free for finite inputs.
...
/// Computes the Euclidean distance between two soil-grid cells.
///
/// The layer and column indices are checked against the fixed model grid. The
/// returned distance uses the row-space width supplied by the caller.
...
/// Maps a vertical distance in centimetres to a fixed soil-layer index.
...
/// Maps a horizontal distance to a fixed soil-column index.
...
/// Returns the ambient-CO₂ correction factor for a calendar year.
