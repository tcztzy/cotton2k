//! Legacy process-wide model variables and compatibility accessors.
//!
//! This module owns the mutable climate, irrigation, and translated legacy
//! variables that the domain crates still exchange through the migration
//! boundary. [`LegacyGlobalState`](crate::LegacyGlobalState) snapshots model
//! values while the runner's global simulation lock serializes access. Direct
//! concurrent mutation of the exposed legacy globals is not supported.
...
#[inline]
/// Returns total leaf weight from the synchronized legacy state.
pub fn TotalLeafWeight() -> f64 {
    global_rust_defs::total_leaf_weight()
}

#[inline]
/// Returns total leaf area from the synchronized legacy state.
