//! Deserializable simulation configuration and profile initialization boundary.
//!
//! [`Profile`] owns site, weather, soil, cultivar, and management inputs plus
//! the initialized [`ModelState`]. Initialization reads weather and soil-input
//! files, prepares legacy arrays, and sets output metadata; output methods then
//! append CSV records from the model state. The root engine drives daily
//! execution, while this module owns profile setup and file I/O errors are
//! returned to the caller.

use crate::input_functions::form;
use crate::root_impedance::RootImpedanceTables;
use crate::*;
...
/// Complete TOML/JSON configuration and initialized runtime state for one run.
pub struct Profile {
    #[serde(skip)]
    /// Source or output profile path used for relative-file resolution.
    pub path: PathBuf,
    /// Canopy light-interception algorithm.
    ...
    pub elevation: f64,
    /// First simulated calendar date.
    #[serde(deserialize_with = "from_isoformat")]
    pub start_date: NaiveDate,
    /// Last simulated calendar date.
    #[serde(deserialize_with = "from_isoformat")]
    pub stop_date: NaiveDate,
    /// Optional measured emergence date.
    #[serde(default)]
    #[serde(deserialize_with = "from_isoformat_option")]
    pub emerge_date: Option<NaiveDate>,
    /// Optional planting date used when emergence is simulated.
    #[serde(default)]
    #[serde(deserialize_with = "from_isoformat_option")]
    pub plant_date: Option<NaiveDate>,
    /// Optional ambient-CO₂ enrichment schedule.
    pub co2_enrichment: Option<CO2Enrichment>,
    /// Optional soil-surface mulch configuration.
    pub mulch: Option<Mulch>,
    /// Path to the daily weather CSV.
    pub weather_path: PathBuf,
    /// Path to the root-impedance CSV, resolved by the runner if absent.
    pub soil_impedance: Option<PathBuf>,
    /// Site-specific weather and thermal coefficients.
    pub site: Site,
    /// Cultivar parameter vector consumed by legacy equations.
    pub cultivar_parameters: Vec<f64>,
    /// Row spacing in centimetres.
    pub row_space: f64,
    /// Optional skip-row width in centimetres.
    #[serde(default = "zero")]
    pub skip_row_width: f64,
    /// Plants per metre of row.
    pub plants_per_meter: f64,
    /// Ordered agronomy operations applied during the run.
    pub agronomy_operations: Vec<AgronomyOperation>,
    /// Parameters required by the `Latered` light-interception method.
    pub light_intercept_parameters: Option<[f64; 20]>,
    /// Initial soil chemistry and water content for fourteen input layers.
    pub soil_layers: [SoilLayer; 14],
    /// Hydraulic properties of the soil horizons.
    pub soil_hydraulic: SoilHydraulic,
    /// Optional observed plant-map records.
    pub plant_maps: Option<Vec<PlantMap>>,
    /// Maximum leaf area index observed during initialization/execution.
    #[serde(skip)]
    pub lmax: f64,
    /// Current moisture-stress reduction applied to photosynthesis.
    #[serde(skip)]
    pub ptsred: f64,
    /// Current ambient-CO₂ correction factor.
    #[serde(skip)]
    pub ambient_CO2_factor: f64,
    #[serde(skip)]
    /// Number of water-table operations in the profile.
    pub num_watertable_data: usize,
    #[serde(skip)]
    #[serde(default = "default_last_day_weather_data")]
    /// Last date available in the loaded weather buffer.
    pub last_day_weather_data: NaiveDate,
    #[serde(skip)]
    /// Root-impedance lookup tables loaded from the profile directory.
    pub root_impedance_tables: Option<RootImpedanceTables>,
    #[serde(skip)]
    /// Runtime model state initialized from this profile.
    pub model_state: ModelState,
}

#[derive(Deserialize, Debug, Clone, Copy)]
/// Calendar-bounded correction to ambient atmospheric CO₂.
pub struct CO2Enrichment {
    /// Multiplicative CO₂ factor.
    pub factor: f64,
    #[serde(deserialize_with = "from_isoformat")]
    /// First date on which enrichment applies.
    pub start_date: NaiveDate,
    #[serde(deserialize_with = "from_isoformat")]
    /// Last date on which enrichment applies.
    pub stop_date: NaiveDate,
}
...
#[derive(Deserialize, Debug, Clone, Copy)]
/// Supported soil-surface mulch layouts.
pub enum MulchType {
    /// No mulch is applied.
    NoMulch,
...
#[derive(Deserialize, Debug, Clone, Copy)]
/// Canopy light-interception algorithm selected for the run.
pub enum LightInterceptMethod {
    /// Original height/row-space relationship.
    Original,
    /// Fry (1980) parameterization.
    Fry1980,
    /// Layered interception parameterization.
    Latered,
}

#[derive(Deserialize, Debug, Clone, Copy)]
/// Soil mulch transmission and active date range.
pub struct Mulch {
    /// Spatial mulch layout.
    pub indicator: MulchType,
    /// Short-wave transmission fraction.
    pub sw_trans: f64,
    /// Long-wave transmission fraction.
    pub lw_trans: f64,
    #[serde(deserialize_with = "from_isoformat")]
    /// Date mulch becomes active.
    pub start_date: NaiveDate,
    #[serde(default)]
    #[serde(deserialize_with = "from_isoformat_option")]
    /// Optional date mulch stops being active.
    pub stop_date: Option<NaiveDate>,
}

#[derive(Deserialize, Debug)]
/// Site coefficients used to reconstruct hourly atmospheric and thermal data.
pub struct Site {
    /// Measured average wind speed when daily wind is absent.
    pub average_wind_speed: Option<f64>,
    /// Dew-point estimates for cool and warm maximum temperatures.
    pub estimate_dew_point: (f64, f64),
    /// Wind onset offset after sunrise in hours.
    pub wind_blow_after_sunrise: f64,
    /// Wind maximum offset after solar noon in hours.
    pub wind_max_after_noon: f64,
    /// Wind stop offset after sunset in hours.
    pub wind_stop_after_sunset: f64,
    /// Night-time wind multiplier.
    pub night_time_wind_factor: f64,
    /// Cloud-type correction factor.
    pub cloud_type_correction_factor: f64,
    /// Maximum-temperature offset after solar noon in hours.
    pub max_temperature_after_noon: f64,
    /// Mean, amplitude, and phase of deep-soil temperature.
    pub deep_soil_temperature: (f64, f64, f64),
    /// Coefficients for estimating dew-point range.
    pub dew_point_range: (f64, f64, f64),
    /// Minimum and maximum albedo bounds.
    pub albedo_range: (f64, f64),
}

#[derive(Deserialize, Debug)]
/// One daily weather record read from the profile's climate CSV.
pub struct WeatherRecord {
    #[serde(deserialize_with = "from_isoformat")]
    /// Calendar date of the record.
    pub date: NaiveDate,
    /// Daily irradiation in MJ m⁻².
    pub irradiation: f64,
    /// Daily maximum air temperature in °C.
    pub tmax: f64,
    /// Daily minimum air temperature in °C.
    pub tmin: f64,
    /// Daily rainfall in model input units.
    pub rain: f64,
    /// Optional daily wind value.
    pub wind: Option<f64>,
    /// Optional daily dew-point temperature in °C.
    pub tdew: Option<f64>,
}
...
#[derive(Deserialize, Debug, Clone, Copy)]
/// Irrigation delivery method encoded in agronomy operations.
...
#[derive(Deserialize, Debug, Clone, Copy)]
/// Fertilizer placement method encoded in agronomy operations.
pub enum FertilizationMethod {
    ...
    /// Foliar placement.
    Foliar = 2,
    /// Drip/fertigation placement.
    Drip = 3,
}

#[derive(Deserialize, Debug, Clone, Copy)]
#[serde(tag = "type")]
/// Ordered irrigation, fertilization, defoliation, cultivation, or water-table action.
pub enum AgronomyOperation {
    /// Applies scheduled or predictive irrigation.
    ...
    fertilization {
        ...
        #[serde(deserialize_with = "from_isoformat")]
        date: NaiveDate,
        /// Urea N applied, kg N per hectare.
        #[serde(default = "zero")]
        urea: f64,
        /// Nitrate N applied, kg N per hectare.
        #[serde(default = "zero")]
        nitrate: f64,
        /// Ammonium N applied, kg N per hectare.
        #[serde(default = "zero")]
        ammonium: f64,
        ...
        #[serde(default = "zero")]
        /// Horizontal drip location in centimetres.
        drip_x: f64,
        #[serde(default = "zero")]
        /// Vertical drip location in centimetres.
        drip_y: f64,
    },
    /// Applies a defoliation treatment.
    defoliation {
        #[serde(deserialize_with = "from_isoformat")]
        /// Date of defoliant application.
        date: NaiveDate,
        /// Target open-boll ratio.
        open_ratio: i32,
        #[serde(default = "default_predict")]
        /// Whether the defoliation date is predicted.
        predict: bool,
        /// Application rate in pints per acre.
        #[serde(default = "zero")]
        ppa: f64,
    },
    /// Applies a cultivation operation at a specified depth.
    cultivation {
        #[serde(deserialize_with = "from_isoformat")]
        /// Date of cultivation.
        date: NaiveDate,
        /// Cultivation depth in centimetres.
        depth: f64,
    },
    /// Supplies one water-table observation.
    watertable(AgronomyOperationWaterTable),
}
#[derive(Deserialize, Debug, Clone, Copy)]
/// Irrigation schedule and optional predictive-irrigation bounds.
...
impl AgronomyOperationIrrigation {
    /// Reports whether this operation asks the engine to predict irrigation.
    pub fn is_predictive(&self) -> bool {
        self.predict
    }

    /// Reports whether predictive irrigation has both required date limits.
    pub fn has_prediction_limits(&self) -> bool {
        self.max_amount.is_some() && self.stop_predict_date.is_some()
    }
}

#[derive(Deserialize, Debug, Clone, Copy)]
/// Water-table level and salinity input for one date.
pub struct AgronomyOperationWaterTable {
    #[serde(deserialize_with = "from_isoformat")]
    /// Date of water-table observation.
    pub date: NaiveDate,
...
#[derive(Deserialize, Debug)]
/// Initial ammonium, nitrate, organic matter, and water content for a layer.
pub struct SoilLayer {
    /// Initial ammonium in kg N per hectare.
    pub ammonium: f64,
    /// Initial nitrate in kg N per hectare.
    pub nitrate: f64,
    /// Initial organic matter percentage.
    pub organic_matter: f64,
    /// Initial water content as a percentage of field capacity.
    pub water_content: f64,
}

#[derive(Deserialize, Debug)]
/// Hydraulic horizon parameters used to initialize the soil grid.
pub struct SoilHydraulic {
    /// Implicit water-flow ratio.
    pub implicit_ratio: f64,
    /// Maximum conductivity cap in model units.
    pub max_conductivity: f64,
    /// Field-capacity matric potential.
    pub psi_fc: f64,
    /// Immediate-drainage matric potential.
    pub psi_id: f64,
    /// Horizons ordered by increasing depth.
    pub layers: Vec<SoilHydraulicLayer>,
}

#[derive(Deserialize, Debug)]
/// Hydraulic and texture parameters for one soil horizon.
pub struct SoilHydraulicLayer {
    /// Cumulative horizon depth in centimetres.
    pub depth: f64,
    /// Air-dry water content.
    pub theta_d: f64,
    /// Saturated water content.
    pub theta_s: f64,
    /// Retention-curve alpha parameter.
    pub alpha: f64,
    /// Retention-curve beta parameter.
    pub beta: f64,
...
#[derive(Deserialize, Debug)]
/// Optional observed plant-map values used by profile initialization or output.
pub struct PlantMap {
    #[serde(deserialize_with = "from_isoformat")]
    /// Date represented by the observation.
    pub date: NaiveDate,
    /// Observed plant height.
    pub plant_height: f64,
    /// Observed main-stem node count.
    pub main_stem_nodes: f64,
    /// Observed square count.
...
impl Profile {
    /// Loads climate and soil inputs and initializes runtime model state.
    ///
    /// This method performs filesystem reads for the weather and root-impedance
    /// files and returns their I/O or parse errors. It must run before daily
    /// the root engine's daily state transitions.
    ...
    /// Appends one output row derived from the current model state.
    ///
    /// The output file is located beside [`Profile::path`]. Filesystem errors
    /// are returned and no in-memory state is changed.
