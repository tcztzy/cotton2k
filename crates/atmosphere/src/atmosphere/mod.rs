//! Atmospheric forcing and radiative-exchange calculations for one simulation day.
//!
//! [`Atmosphere`] derives solar geometry from a date and site coordinates, then
//! converts daily climate records into hourly temperature, radiation, humidity,
//! wind, and evapotranspiration values. The `meteorology` entry point mutates
//! the shared [`crate::ModelState`] and reads climate data from the legacy climate
//! buffer; it performs no file I/O and relies on the caller's simulation lock.
...
/// Solar geometry and daily atmospheric forcing for one calendar date.
pub struct Atmosphere {
    ...
    /// Daylight duration computed from latitude and declination.
...
impl Atmosphere {
    /// Computes solar geometry and extraterrestrial radiation for one date.
    ///
    /// `longitude` and `latitude` are expressed in decimal degrees. The
    /// resulting times use the geographically inferred fixed UTC offset.
    ...
    /// `Clim[400]`;    -- defined in global state
    ...
    /// Converts daily climate records into hourly atmospheric state.
    ///
    /// The profile supplies site coefficients and the model state receives
    /// hourly temperature, radiation, humidity, wind, and evapotranspiration
    /// values. Climate records are read from the shared `Clim` buffer.
...
/// Computes air-vapor pressure in kPa from air temperature in °C.
...
///
/// `vp` is vapor pressure in kPa and `tk` is air temperature in kelvin.
///
/// References:
