...
//     This function executes output of plant maps. It plots the cotton plant
//     with the
//  corresponding symbols at the correct locations. It is called from function
//  DailyOutput().
//     It calls bollsize() and sitecode().
//
//     The following global and file scope variables are referenced here:
//       bsize, Date, FruitingCode, mcode, NumFruitBranches, NumNodes,
//       NumVegBranches, ProfileName.
...
//        (4) - Average boll size (bsize); function bollsize() is called.
{
    ...
    std::string char3[13] = {"-1", "-2", "-3", "-4", "-5", "-6", "-7",
                             "-8", "-9", "-0", "-A", "-$", "-X"};
    std::string char4[13] = {"1-", "2-", "3-", "4-", "5-", "6-", "7-",
                             "8-", "9-", "0-", "A-", "$-", "X-"};
    ...
    for (int k = 0; k < 3; k++)
        for (int l = 0; l < 30; l++) {
            pri[k][l] = ' ';
            for (int m = 0; m < 5; m++) prt[k][l][m] = "  ";
        }
    //     Loop over all vegetative stems. Separate loops over fruiting branches
    //     and nodes
    //  for odd numbered fruiting branches (right side of the plotted plant) and
    //  even numbered fruiting branches (left side).
        ...
        for (int l = 0; l < nbrch; l += 2) {
            ...
            for (int m = 0; m < nnid; m++) {
                if (FruitingCode[k][l][m] > 0) {
                    //     Array prt for all existing sites is set with symbols
                    //     for plotting. mcode is assigned
                    //  symbol sets char1 and char2, whereas bsize is assigned
                    //  symbol sets char3 and char4, for right and left sides,
                    //  respectively.
                    //     'X' is a square, 'A' is an absciced site, '$' is an
                    //     open boll. For plotting mcode,
                    //  symbols 'B' and '*' are young and full sized green
                    //  bolls, respectively. For boll size plots, symbols '0' to
                    //  '9' are used for size groups.
                    //     Array pri for all existing monopodial stem nodes is
                    //     assigned symbol chari ('I').
                    if (mtype == 1) {
                        icode = mcode[k][l][m];
                        if (icode > 0) prt[k][l][m] = char1[icode - 1];
                    } else if (mtype == 4) {
                        icode = (int)(bsize[k][l][m] + 0.5);
                        if (icode > 9) icode = 9;
                        if (bsize[k][l][m] < 0.5) icode = 10;
                        if (FruitingCode[k][l][m] == 4 ||
                            FruitingCode[k][l][m] == 5)
                            icode = 11;
                        if (FruitingCode[k][l][m] == 1) icode = 13;
                        if (icode > 0) prt[k][l][m] = char3[icode - 1];
                    }
                    pri[k][l] = chari;
                }  // if FruitingCode
            }      // for m
        ...
        for (int l = 1; l < nbrch; l += 2) {
            ...
            for (int m = 0; m < nnid; m++) {
                ...
                if (FruitingCode[k][l][m] > 0) {
                    if (mtype == 1) {
                        icode = mcode[k][l][m];
                        if (icode > 0) prt[k][l][m0] = char2[icode - 1];
                    } else if (mtype == 4) {
                        icode = (int)(bsize[k][l][m] + 0.5);
                        if (icode > 9) icode = 9;
                        if (bsize[k][l][m] < 0.5) icode = 10;
                        if (FruitingCode[k][l][m] == 4 ||
                            ...
                            icode = 11;
                        if (FruitingCode[k][l][m] == 1) icode = 13;
                        if (icode > 0) prt[k][l][m0] = char4[icode - 1];
                    }
                    pri[k][l] = chari;
                }  // if FruitingCode
            }      // for m
    ...
    if (nbrch % 2 != 0) {
        for (int l = 0; l < nbrch; l += 2) {
            ...
            lx1 = lx - 1;
            File24 << "           " << pri[0][lx];
            for (int m = 0; m < 5; m++) File24 << prt[0][lx][m];
            File24 << "            " << pri[1][lx];
            for (int m = 0; m < 5; m++) File24 << prt[1][lx][m];
            File24 << "            " << pri[2][lx];
            for (int m = 0; m < 5; m++) File24 << prt[2][lx][m];
            File24 << endl;
            if (lx1 >= 0) {
                File24 << " ";
                for (int m = 0; m < 5; m++) File24 << prt[0][lx1][m];
                File24 << pri[0][lx1] << "            ";
                for (int m = 0; m < 5; m++) File24 << prt[1][lx1][m];
                File24 << pri[1][lx1] << "            ";
                for (int m = 0; m < 5; m++) File24 << prt[2][lx1][m];
...
////////////////////////
void sitecode()
//     This function is called from function cotplt(). It sets mcode to draw the
//     plant map.
//  A fruit will be included in the average plant map if the fraction of fruit
//  remaining at each site is equal to or larger then the average retention for
//  each fruit class.
//
...
//     The average retention is computed from the data for these groups.
{
    ...
    //     Loop over all possible fruiting sites. The number of retained squares
    //     or bolls,
    //  as well as the number of sites, are computed for each group of fruits
    //  (1, 2, 3, and 7, as classified for FruitingCode).
    for (int k = 0; k < NumVegBranches; k++) {
        int nbrch;  // the number of fruiting branches on a vegetative stem.
        nbrch = NumFruitBranches[k];
    ...
    double avgfru1 = 0, avgfru2 = 0, avgfru3 = 0,
           avgfru7 = 0;  // average retention (per site)
    //       of squares, old green bolls, open bolls, and young green bolls,
    //       respectively.
    if (fru1no > 0) avgfru1 = sumfru1 / fru1no;
    if (fru2no > 0) avgfru2 = sumfru2 / fru2no;
    if (fru3no > 0) avgfru3 = sumfru3 / fru3no;
    if (fru7no > 0) avgfru7 = sumfru7 / fru7no;
    //     Loop over all possible fruiting sites, and check for existence of
    //     square or boll.
    //  If it is a square, define mcode = 1 if retention is above the average,
    //  mcode = 4 if below. If it is a full sized green boll, define mcode = 2
    //  if  retention is above the average, mcode = 5 if below. If it is an open
    //  boll, define mcode = 3 if retention is above the average, mcode = 5 if
    //  below. If it is a young green boll, define mcode = 7 if retention is
    //  above the average, mcode = 5 if below. For abscised squares or bolls
    //  (fcode is 4 or 5) define mcode as 4 or 5, respectively.
    for (int k = 0; k < NumVegBranches; k++) {
...
//     The following fie scope variable is set here:    bsize
{
    ...
    //     Compute boll size, bsize, as the average weight of seed cotton per
    //     boll, for each site
    //  with a boll. bsize will be zero when no bolls are retained. It can not
    //  be larger than a maximum value defined by VarPar(11).
    for (int k = 0; k < NumVegBranches; k++) {
        int nbrch;  // the number of fruiting branches on a vegetative stem.
        nbrch = NumFruitBranches[k];
        for (int l = 0; l < nbrch; l++) {
            ...
            nnid = NumNodes[k][l];
            for (int m = 0; m < nnid; m++) {
                if (FruitingCode[k][l][m] == 2 || FruitingCode[k][l][m] == 3 ||
                    FruitingCode[k][l][m] == 7) {
                    if (FruitFraction[k][l][m] > 0.001)
                        bsize[k][l][m] =
                            BollWeight[k][l][m] / FruitFraction[k][l][m];
                    else
                        bsize[k][l][m] = 0;
