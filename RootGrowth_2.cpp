...
//     This function is called from ActualRootGrowth(). It updates the variable
//     celage(l,k)
//  for the age of roots in each soil cell containing roots. When root age
//  reaches a threshold thtrn(i), a transformation of root tissue from class i
//  to class i+1 occurs. The proportion transformed is trn(i).
...
{
    //     The following constant parameters are used:
    const double thtrn[2] = {20.0,
                             40.0};  // the time threshold, from the initial
    ...
    const double trn[2] = {0.0060,
                           0.0050};  //  the daily proportion of this transfer.
                                     //
...
//     The following global variable is set here:     TotalRootWeight
{
    ...
    if (OutIndex[22] > 0) {
        ...
        if ((Kday % nni) == 0 || Kday == 1) {
            double
                rootl[maxl];  // weight of all root classes in a soil layer, g.
            double rootli[maxl]
                         [3];  // weight of roots of class i in a soil layer, g.
            double rwt[3];     // total weight of roots of class i, g per slab.
            for (int i = 0; i < 3; i++) rwt[i] = 0;
            for (int l = 0; l < NumLayersWithRoots; l++) {
                rootl[l] = 0;
                for (int i = 0; i < 3; i++) rootli[l][i] = 0;
                for (int k = 0; k < nk; k++)
                    for (int i = 0; i < 3; i++) {
            ...
            File34 << endl << endl;
            //    Compute and write (to file 34) the roots capable of uptake in
            //    mg
            //  per cm**3 soil volume for each cell.
            File34 << "  Root weight capable of uptake (in mg / cm**3 soil "
                      "volume) :"
                   << endl;
            File34 << " Layer ";
            for (int i = 0; i < nk; i++) {
