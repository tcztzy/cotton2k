...
//       bEnd, SoilTemp, FoliageTemp, MulchTemp.
{
    ...
    const double wndfac =
        0.60;  // Ratio of wind speed under partial canopy cover.
    const double cswint =
        0.75;  // proportion of short wave radiation (on fully
    ...
    if (VolWaterContent[0][k] <= thad[0])
        ...
        ag = SitePar[16] + (SitePar[15] - SitePar[16]) *
                               (FieldCapacity[0] - VolWaterContent[0][k]) /
    ...
    double rzero =
        Radiation[ihr] / 41880;  // short wave (global) radiation (ly / sec).
    double rss0 =
        rzero *
        (1 - sf * cswint);  // global radiation after passing through canopy
        ...
        rsm = rss0 * (1 - MulchTranSW)        // absorbed by mulch
              + rss0 * MulchTranSW * ag *
                    (1 - MulchTranSW);  // reflected up from soil surface and
                                        // absorbed by mulch
        rsup = rss0 * MulchTranSW * ag *
               MulchTranSW;  // reflected up from soil surface through mulch
    } else {
        tm = 0;
    ...
    double vp = 0.01 * RelativeHumidity[ihr] *
                VaporPressure(AirTemp[ihr]);  // air vapor pressure, KPa.
    double ea0 = clearskyemiss(
        vp, thet);  // sky emissivity from clear portions of the sky.
    double rlzero;  // incoming long wave radiation (ly / sec).
    rlzero = (ea0 * (1 - CloudCoverRatio[ihr]) + CloudCoverRatio[ihr]) *
                 stefa1 * pow(thet, 4) -
             CloudTypeCorr[ihr] /
                 41880;  // CloudTypeCorr converted from W m-2 to ly sec-1.
                         //
    ...
    if (sf >= 0.05)  // a shaded soil column
    {
        ...
        rsv = rzero * (1 - albedo[ihr]) * sf * cswint  //  from above
              + rsup * (1 - albedo[ihr]) * sf *
                    cswint;  //  reflected from soil surface
        ...
        varcc = SensibleHeatTransfer(tv, tafk, PlantHeight,
                                     wndhr);  // canopy to air
        if (bEnd) return;
        rocp = 0.08471 / tafk;
    ...
    do {
        ...
        if (bMulchon) {
            ...
            tmold = tm;
            bEnd = SoilMulchBalance(ihr, k, rlzero, rsm, rss, sf, so, so2, so3,
                                    thet, tm, tv, wndcanp);
            if (bEnd) return;
        } else {
            ...
            if (bEnd) return;
            rocp = 0.08471 / tafk;
            double hsg = rocp * varc;  // multiplier for computing sensible heat
                                       // transfer soil to air.
            //     Call SoilSurfaceBalance() for energy balance in soil surface
            //     / air interface.
            SoilSurfaceBalance(ihr, k, ess, rlzero, rss, sf, hsg, so, so2, so3,
                               thet, 0, tv);
            if (bEnd) return;
        }
        //
...
//     This function solves the energy balance equations at the interface of the
//     soil
//  surface and the plastic mulch cover and computes the resulting temperatures
//  of the soil surface and of the plastic mulch.
//     it is called from EnergyBalance(), on each time step and for each soil
//     column, if
//  this column is covered with a plastic mulch.  It calls functions
//  SensibleHeatTransfer(), SoilSurfaceBalance() and MulchSurfaceBalance().
//     Units for all energy fluxes are: cal cm-2 sec-1.
//     If the return value is true, it means there was an error and simulation
//     will end.
//
//     The following global variables are referenced here:
...
//
{
    ...
    const double eg = 0.95;          // emissivity of the soil surface.
    const double stefa1 = 1.38e-12;  // stefan-boltsman constant.
    //     Compute long wave radiation reaching the surface mulch from above,
    //     and
    ...
    if (sf > 0.05)  // shaded column
    {
        rlsp0 = (1 - sf) * (1 - MulchTranLW) *
                    rlzero  // from sky in unshaded segment
                + sf * (1 - MulchTranLW) * ef * stefa1 *
                      pow(tv, 4);  // from foliage in shaded segment
        tafk = (1 - sf) * thet + sf * (0.1 * tm + 0.3 * thet + 0.6 * tv);
    ...
    //     rls5 is the multiplier of tm**4 for emitted long wave radiation from
    //     mulch,
    ...
    //     Call SensibleHeatTransfer() to compute sensible heat transfer between
    //     plastic mulch and air
    double varcm;  // sensible heat transfer coefficients for mulch to air
                   // (before multiplying by ROCP).
    varcm = SensibleHeatTransfer(tm, tafk, 0, wndcanp);
    if (bEnd) return true;
    //
    double hsgm;  // multiplier for computing sensible heat transfer from soil
                  // to mulch.
    double hsgp;  // multiplier for computing sensible heat transfer from mulch
                  // to air.
    double rocp;  // air density * specific heat at constant pressure [= 0.24 *
                  // 2 * 1013 / (5740 * tk) ]
    rocp = 0.08471 / tafk;
    hsgp = rocp * varcm;
    //     Compute sensible heat transfer between plastic mulch and soil surface
    if (tm > 0) rocp = 0.08471 / tm;
    ...
    do {
        ...
        SoilSurfaceBalance(ihr, k, 0, rlzero, rss, sf, hsgm, so, so2, so3, thet,
                           tm, tv);
        if (bEnd) return true;
        ...
        hsgm = 2 * rocp * fabs(so - tm);
        MulchSurfaceBalance(ihr, k, rlsp, rls5, rsm, sf, hsgp, hsgm, so, thet,
                            tm, tv);
        if (bEnd) return true;
