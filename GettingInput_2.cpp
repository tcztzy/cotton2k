...
//        VolNo3NContent, VolUreaNContent, VolWaterContent
{
    ...
    for (int l = 0; l < nl; l++) {
        ...
        sumdl += dl[l];
        while ((sumdl > ldepth[j]) && (j < lyrsol)) j++;
        SoilHorizonNum[l] = j;
        ...
        if (thetas[j] > PoreSpace[l]) thetas[j] = PoreSpace[l];
        thad[l] = airdr[j];
        thts[l] = thetas[j];
        ...
        if (SaturatedHydCond[j] <= 0)
            SaturatedHydCond[j] = condfc[j] / wcond(FieldCapacity[l], thad[l],
                                                    thts[l], beta[j], 1, 1);
    }
    ...
    for (int l = 0; l < nl; l++) {
        ...
        if (VolWaterContent[l][0] < airdr[n]) VolWaterContent[l][0] = airdr[n];
        ...
        if (rnno3[j] < 2.0) rnno3[j] = 2.0;
        if (rnnh4[j] < 0.2) rnnh4[j] = 0.2;
