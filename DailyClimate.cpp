...
// files "global.h" and "global.cpp" (which also contains a "Dictionary" of
// these variables).
...
//
{
    ...
    double cd = cos(xlat) *
                cos(declination);  // amplitude of the sine of the solar height.
    double sd =
        sin(xlat) *
        sin(declination);  // seasonal offset of the sine of the solar height.
    ...
    if (fabs(sd / cd) >= 1)
        ...
        //     sunset,
        double dsbe = acos(-sd / cd) * 24 / pi *
                          (sd + c11 * sd * sd + 0.5 * c11 * cd * cd) +
                      12 * (cd * (2 + 3 * c11 * sd)) *
                          sqrt(1 - (sd / cd) * (sd / cd)) / pi;
    ...
    double t2 =
        SolarNoon + SitePar[2];     // the hour at which wind speed is maximum
                                    // (SitePar(2) hours after solar noon).
    double t3 = suns + SitePar[3];  // the hour at which wind stops to blow
                                    // (SitePar(3) hours after sunset).
    double wnytf = SitePar[4];      // used for estimating night time wind (from
...
//
{
    ...
    tmpisr = 1367 * (1.00011 + .034221 * cos(xday) + .00128 * sin(xday) +
                     .000719 * cos(2 * xday) + .000077 * sin(2 * xday));
    ...
    double exday = (.000075 + .001868 * cos(xday) - .032077 * sin(xday) -
                    .014615 * cos(2 * xday) - .04089 * sin(2 * xday)) *
                   12. / pi;
...
{
    const double tkk =
        15;  // The temperature increase at which the sensible heat flux is
             //  doubled, in comparison with the situation without buoyancy.
    const double tcoef =
        4;  // time coefficient for the exponential part of the equation.
    ...
    if (ip1 > LastDayWeatherData) ip1 = Daynum;
    //
...
//
{
    ...
    double vpa =
        VaporPressure(td);  // the actual vapor pressure in the air (mbar).
    double RelHumHour =
        100 * vpa / esvp;  // relative humidity at this time of day, %.
    if (RelHumHour < 1) RelHumHour = 1;
    if (RelHumHour > 100) RelHumHour = 100;
    return RelHumHour;
    //     Reference:
    //     Ephrath, J.E., Goudriaan, J. and Marani, A. 1996. Modelling
    //  diurnal patterns of air temperature, radiation, wind speed and
...
//
{
    ...
    double wmin =
        wind *
        wnytf;  //  the constant minimum wind speed during the night (m/sec).
    double wtday =
        wind - wmin * 3.6 *
                   24;  //  integral of wind run from t1 to t3, minus wmin (km).
    double wmax =
        wtday * 2 * pi / 3.6 /
        (sf1 + sf2);  //  the maximum wind speed (m per sec), above wmin.
                      //
    if (ti >= t1 && ti < t2)
...
//     VaporPressure() computes the water vapor pressure in the air (in KPa
//     units) as a
...
{
    const double stefb = 5.77944E-08;  // the Stefan-Boltzman constant, in W m-2
                                       // K-4 (= 1.38E-12 * 41880)
    ...
    for (int ihr = 0; ihr < 24; ihr++) {
        ...
        if ((cosz >= 0.1736) && (iamhr == 0)) iamhr = ihr;
        if ((ihr >= 12) && (cosz <= 0.1736) && (ipmhr == 0)) ipmhr = ihr - 1;
    ...
    for (int ihr = 0; ihr < 24; ihr++)  //  2nd hourly loop
    {
        ...
        double svp =
            VaporPressure(AirTemp[ihr]);  // saturated vapor pressure, mb
        ...
        //  radiation.
        double tk =
            AirTemp[ihr] + 273.161;  // hourly air temperature in Kelvin.
        double ea0 = clearskyemiss(
            vp, tk);  // clear sky emissivity for long wave radiation
                      //     Compute incoming long wave radiation:
        double rlonin =
            (ea0 * (1 - CloudCoverRatio[ihr]) + CloudCoverRatio[ihr]) * stefb *
                pow(tk, 4) -
            CloudTypeCorr[ihr];
        rnet[ihr] =
            (1 - albedo[ihr]) * Radiation[ihr] + rlonin - stefb * pow(tk, 4);
        Rn += rnet[ihr];
        ...
        double w = del(tk, svp) /
                   (del(tk, svp) +
                    gam(Elevation,
                        AirTemp[ihr]));  // coefficient of the Penman equation
        ...
        if (ReferenceETP[ihr] < 0) ReferenceETP[ihr] = 0;
...
//     Function clearskyemiss() estimates clear sky emissivity for long wave
//     radiation.
//  Input arguments:
...
//
{
    ...
    if (isrhr > 0) rasi = Radiation[ihr] / isrhr;
    if (coszhr >= 0.1736 && rasi >= 0.375) {
        double angle = pi * (ihr - SolarNoon + 0.5) /
                       DayLength;  // hour angle (from solar noon) in radians.
...
//     Function gam() computes the psychometric constant at elevation (elev), m
//     above
...
//
{
    double bp =
        101.3 - .01152 * elev +
        5.44e-07 *
            pow(elev, 2);  //  barometric pressure, KPa, at this elevation.
...
//
{
    ...
    double refalb = p4;  // the reference albedo
                         //
    if (isrhr > 0) rasi = rad / isrhr;
    if (coszhr > 0.1736 && rasi >= 0.375) {
        refalb = p1 * sunahr + p2 * exp(-p3 * sunahr);
        if (refalb > p4) refalb = p4;
    }
    return refalb;
}
//      Reference:
...
//
{
    ...
    double hrangle =
        15 * (ti - SolarNoon) * pi / 180;  // hourly angle converted to radians
...
{
    static bool bFirst =
        true;  // if this is the first time the function is called.
    static int
        iGroup;  // soil group number (by clay and sand in upper soil layer)
    ...
    int i01 = Daynum - 5;
    if (i01 < DayStart) i01 = DayStart;
    int i02 = Daynum;
    double PreviousWetting =
        0;  // five day total (before this day) of rain and irrigation, mm
    ...
    double crvnum =
        78.0;  // Runoff curve number, unadjusted for moisture and soil type.
