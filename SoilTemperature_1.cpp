...
//       DayEmerge, Daynum, isw, LeafAreaIndex, PlantHeight, PlantRowColumn, nk,
//       RowSpace.
//     The following global variables are set here:    LightIntercept, rracol.
{
    ...
    for (int k = 0; k < nk; k++) {
        ...
        //     Relative shading is computed as a function of sw1 and plant
        //     height, modified by LightIntercept.
        //  rracol is the fraction of radiation received by a soil column. Iys
        ...
        rracol[k0] = 1 - shade;
        if (rracol[k0] < 0.05) rracol[k0] = 0.05;
    }  // end for k
}
////////////////////////////////////////////////////////////////////////////////////////////////
void SoilTemperature()
//     This is the main part of the soil temperature sub-model. It is called
//     daily from
...
//       SoilTemp, SoilTempDailyAvrg, VolWaterContent,
{
    ...
    double dts =
        2 * pi * SitePar[10] / 365 * cos(2 * pi * (Daynum - SitePar[11]) / 365);
    ...
    if (isw > 1) {
        ...
        int nshadedcol =
            0;  // number of at least partially shaded soil columns.
        kk = nk;
        for (int k = 0; k < nk; k++)
            if (rracol[k] <= 0.99) {
                shadetot += 1 - rracol[k];
        ...
        //
        if (nshadedcol > 0) shadeav = shadetot / nshadedcol;
    }
    //     Set daily averages of soil temperature to zero.
    for (int l = 0; l < nl; l++)
        for (int k = 0; k < nk; k++) SoilTempDailyAvrg[l][k] = 0;
    ...
    for (int ihr = 0; ihr < iter1; ihr++) {
        ...
        for (int k = 0; k < kk; k++) {
            ...
            //     relative shading.
            double etp1 =
                0;  // actual hourly transpiration (mm s-1) for a column.
            if (shadeav > 0.000001) etp1 = etp0 * (1 - rracol[k]) / shadeav;
            //     Check if mulch is on for this date and for this column.
            bool bMulchon;  // is true if this column is covered with plastic
                            // mulch now, false if not.
            if (MulchIndicator == 0 || Daynum < DayStartMulch ||
                Daynum > DayEndMulch)
                bMulchon = false;
            else {
                if (MulchIndicator == 1)
            ...
            double ess = 0;  //   evaporation rate from surface of a soil column
                             //   (mm / sec).
            if (bMulchon == false) {
                //     The potential evaporation rate (escol1k) from a column is
                //     the sum of the radiation
                ...
                double escol1k;  // potential evaporation fron soil surface of a
                                 // column, mm per hour.
                escol1k = es1hour[ihr] * rracol[k] + es2hour[ihr];
                es += escol1k * wk[k];
                //     Compute actual evaporation from soil surface. update
                //     VolWaterContent of
                //  the soil soil cell, and add to daily sum of actual
                //  evaporation.
                double evapmax = 0.9 * (VolWaterContent[0][k] - thad[0]) * 10 *
                                 dl[0];  // maximum possible evaporatio from a
                                         // soil cell near the surface.
                if (escol1k > evapmax) escol1k = evapmax;
                VolWaterContent[0][k] -= 0.1 * escol1k / dl[0];
                ActualSoilEvaporation += escol1k * wk[k];
                ess = escol1k / dlt;
            }
            //     Call EnergyBalance to compute soil surface and canopy
            //     temperature.
        ...
        double tsolav[maxl];  // hourly average soil temperature C, of a soil
                              // layer.
        //     Loop over kk columns, and call SoilHeatFlux().
        for (int k = 0; k < kk; k++) SoilHeatFlux(dlt, iv, nn, layer, k);
        //     If no horizontal heat flux is assumed, make all array members of
        //     SoilTemp equal to the
        //  value computed for the first column. Also, do the same for array
        //  memebers of VolWaterContent.
        if (isw <= 1)
            for (int l = 0; l < nl; l++)
                for (int k = 1; k < nk; k++) {
                    SoilTemp[l][k] = SoilTemp[l][0];
                    if (l == 0) VolWaterContent[l][k] = VolWaterContent[l][0];
                }
        //
        ...
        //     Compute average temperature of foliage, in degrees C. The average
        //     is weighted by the canopy
        //  shading of each column, only columns which are shaded 5% or more by
        //  canopy are used.
        double tfc = 0;  // average foliage temperature, weighted by shading in
                         // each column
        double shading =
            0;  // sum of shaded area in all shaded columns, used to compute TFC
        for (int k = 0; k < nk; k++) {
            if (rracol[k] <= 0.95) {
       ...
       //      At the end of the day compute actual daily evaporation and its
       //      cumulative sum.
    if (kk == 1) {
        es = es / wk[1];
...
//  DeepSoilTemperature, SoilTemp.
{
    ...
    int idd = Daynum - 4 -
              DayStart;  // number of days minus 4 from start of simulation.
    if (idd < 0) idd = 0;
    double tsi1 =
        0;  // Upper boundary (surface layer) initial soil temperature, C.
    for (int i = idd; i < idd + 5; i++) tsi1 += Clim[i].Tmax + Clim[i].Tmin;
    tsi1 = tsi1 / 10;
    ...
    DeepSoilTemperature =
        SitePar[9] + SitePar[10] * sin(2 * pi * (Daynum - SitePar[11]) / 365);
    ...
    for (int l = 0; l < nl; l++) {
        ...
        for (int k = 0; k < nk; k++) SoilTemp[l][k] = tsi;
    }
}
