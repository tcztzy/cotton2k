...
//     This function simulates the application of nitrogen fertilizer on each
//     date
...
//     The following global variables are set here:
//       CumFertilizerN, LeafNitrogen, VolNh4NContent, VolNo3NContent,
//       VolUreaNContent.
{
    const double ferc =
        0.01;  //  constant used to convert kgs per ha to mg cm-2
    ...
    for (int i = 0; i < NumNitApps; i++) {
        ...
        if (Daynum == NFertilizer[i].day) {
            //     Compute the sum of mineral N applied as fertilizer
            //     (CumFertilizerN, in mg per slab).
            CumFertilizerN += ferc * RowSpace *
                              (NFertilizer[i].amtamm + NFertilizer[i].amtnit +
                               NFertilizer[i].amtura);
            //     If this is a BROADCAST fertilizer application:
            if (NFertilizer[i].mthfrt == 0) {
                //     Compute the number of layers affected by broadcast
                //     fertilizer incorporation (lplow),
                ...
                //     Calculate the actual depth of fertilizer incorporation in
                //     the soil (fertdp) as the sum of
                //  all soil layers affected by incorporation.
                double fertdp =
                    0;  // depth of broadcast fertilizer incorporation, cm
                for (int l = 0; l < lplow; l++) fertdp += dl[l];
                //     Update the nitrogen contents of all soil soil cells
                //     affected by this fertilizer application.
                for (int l = 0; l < lplow; l++)
                    for (int k = 0; k < nk; k++) {
                        VolNh4NContent[l][k] +=
                            NFertilizer[i].amtamm * ferc / fertdp;
                        VolNo3NContent[l][k] +=
                            NFertilizer[i].amtnit * ferc / fertdp;
                        VolUreaNContent[l][k] +=
                            NFertilizer[i].amtura * ferc / fertdp;
                    }
            }
            //     If this is a FOLIAR fertilizer application:
                ...
                for (int k = 0; k < nk; k++) {
                    VolNh4NContent[0][k] += NFertilizer[i].amtamm *
                                            (1 - 0.70 * LightIntercept) * ferc /
                                            dl[0];
                    VolNo3NContent[0][k] +=
                        NFertilizer[i].amtnit * ferc / dl[0];
                    VolUreaNContent[0][k] += NFertilizer[i].amtura *
                                             (1 - 0.70 * LightIntercept) *
                                             ferc / dl[0];
                }
            }
            //     If this is a SIDE-DRESSING of N fertilizer:
            else if (NFertilizer[i].mthfrt == 1) {
                //     Define the soil column (ksdr) and the soil layer (lsdr)
                //     in
                //  which the side-dressed fertilizer is applied.
                int ksdr = NFertilizer[i].ksdr;  // the column in which the
                                                 // side-dressed is applied
                int lsdr = NFertilizer[i].ksdr;  // the layer in which the
                                                 // side-dressed is applied
                int n00 = 1;  // number of soil soil cells in which side-dressed
                              // fertilizer is incorporated.
                //     If the volume of this soil cell is less than 100 cm3, it
                //     is assumed that the fertilizer
                //  is also incorporated in the soil cells below and to the
                //  sides of it.
                ...
                double addamm;  // amount of ammonium N added to the soil by
                                // sidedressing (mg per cell)
                double addnit;  // amount of nitrate N added to the soil by
                                // sidedressing (mg per cell)
                double addnur;  // amount of urea N added to the soil by
                                // sidedressing (mg per cell)
            ...
            //     If this is FERTIGATION (N fertilizer applied in drip
            //     irrigation):
            else if (NFertilizer[i].mthfrt == 3) {
                //      Convert amounts added to mg cm-3, and update the
