...
! ---------------
  ...
  if (LSTAT=='OLD'.and.LPRIV /= ' ') then
     call WARNING ('FOPENG', 'status OLD does not require a privilege')
...
! =====================================
  if (LSTAT == 'OLD') then
     if (STR) OPEN (IUNIT, FILE=LFNAME(1:ILFIL), STATUS=LSTAT, ACTION='READ', ACCESS='STREAM'    , FORM=FFF(1:ILF), IOSTAT=IOS)
     if (SEQ) OPEN (IUNIT, FILE=LFNAME(1:ILFIL), STATUS=LSTAT, ACTION='READ', ACCESS='SEQUENTIAL', FORM=FFF(1:ILF), IOSTAT=IOS)
     if (DIR) OPEN (IUNIT, FILE=LFNAME(1:ILFIL), STATUS=LSTAT, ACTION='READ', ACCESS='DIRECT'    , FORM=FFF(1:ILF), IOSTAT=IOS, &
                           RECL=IWL)
  else
