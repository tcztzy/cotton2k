! File VersionID:
!   $Id: macropore.f90 311 2016-12-22 12:17:11Z kroes006 $
! ----------------------------------------------------------------------
!
!
!************************************************************************
...
! --- calculate relative reference KD = KDCrRlRef 
            ...
            do 69 ic= ICpHRef, ICpBot
               ...
               if (SwSoilShr(il).ne.0) then
                  ...
                  ThetHydrStat = watcon(ic,HHydrStat,cofgen,            &
...
! --- Calculate Theta(h) and Diffusivity(h) for the relevant range: pF4.2 to h = 0 
      ThetpF4_2= watcon(Node,-1.6d4,CofGen,swsophy,numtab,sptab,        &
     &                 ientrytab)
     ...
     &          ientrytab)
         K_h = hconduc (Node,Head,Thet(it),cofgen,0,Dum,swsophy,numtab, &
     &          sptab,ientrytab)
         Difmoiscap = moiscap (Node,Head,CofGen,dt,swsophy,numtab,sptab,&
     &          ientrytab)
...
!     WthMpMin = 0.01 cm of macropores; below this width, pores are no macropores.  
      ArMpSsMin= 1.d0 - (1.d0 - WthMpMin/DiPoCp(1))**2
...
!   - overall capacity for vertical water inflow at soil surface: KsMpSs 
      KsMpSs= 2.d0*7.2d+8 * (DiPoCp(1)*(1.d0-dsqrt(1.d0-ArMpSs)))**3/&
     &                       DiPoCp(1)
