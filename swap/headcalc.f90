! File VersionID:
!   $Id: headcalc.f90 328 2017-06-03 21:05:50Z kroes006 $
! ----------------------------------------------------------------------
      subroutine headcalc 
! ----------------------------------------------------------------------
...
! --- local
      ...
      real(8)   factmax, sink(macp), sum, sum1, sumold, deviat
      real(8)   hold(macp)
...
!     groundwater level  specified
     ...
     &                       swpondmx,pondmxtab,t1900)
            ...
            theta(1) = watcon(1,gwlinp,cofgen,                          &
     &                        swsophy,numtab,sptab,ientrytab)
            kmean(1) = hconduc(1,gwlinp,theta(1),cofgen,                &
     &                   swfrost,rfcp(1),swsophy,numtab,sptab,ientrytab)
!           in case of static macropores FrArMtrx < 1
     ...
     &                          / dt+ sink(i) +qrot(i)
            ...
            if(SwKimpl.eq.1)then
               do i=1,numnod
                  k(i) = hconduc(i,h(i),theta(i),cofgen,                &
     &                   swfrost,rfcp(i),swsophy,numtab,sptab,ientrytab)
...
!     node nr of compartment with minimized flux of capillary rise
      ...
      do i = 1,numnod
         k(i) = hconduc(i,h(i),theta(i),cofgen,swfrost,rfcp(i),         &
     &                  swsophy,numtab,sptab,ientrytab)
!
         if (swcaprise) then
!
!           Prevent capillary rise into the root zone !! special for experts only
     ...
     &          qrot(i) - kmean(i) * hgrad(i) + kmean(i+1) * hgrad(i+1)
      ...
      if(swbotb.eq.1 .and. (.not.fllowgwl))then
         theta(NN)= watcon(NN,h(NN),cofgen,swsophy,numtab,sptab,        &
     &              ientrytab)
         k(NN)    = hconduc(NN,h(NN),theta(NN),cofgen,                  &
     &                  swfrost,rfcp(NN),swsophy,numtab,sptab,ientrytab)
!        in case of static macropores FrArMtrx < 1
...
! ---       extra groundwater flux might be added
            ...
            kmean(numnod+1) = hconduc(numnod,h(numnod),theta(numnod),   &
     &       cofgen,swfrost,rfcp(numnod),swsophy,numtab,sptab,ientrytab)
            if(swmacro.eq.1) then 
...
!        as part of main diagonal
            ...
            dimoca(i) = moiscap(i,h(i),cofgen,dt,swsophy,numtab,sptab,  &
     &                          ientrytab)
         ...
         if(SwKimpl.eq.1)then
            do i = 1, NN
               dkdh(i)= dhconduc(i,h(i),theta(i),dimoca(i),cofgen,     &
     &                   swfrost,rfcp(i),swsophy,numtab,sptab,ientrytab)
...
!           Newton Raphson step
            ...
            do i = 1,NN
              theta(i) = watcon(i,h(i),cofgen,swsophy,numtab,sptab,     &
     &                          ientrytab)
            enddo
            do i=2,NN
               hgrad(i) = (h(i-1)-h(i))/disnod(i) + 1.0d0
...
!
     ...
     &              + qrot(i) - kmean(i)*hgrad(i)+kmean(i+1)*hgrad(i+1)
            ...
            if(swbotb.eq.1 .and. (.not.fllowgwl))then
               theta(NN) = watcon(NN,h(NN),cofgen,swsophy,numtab,sptab, &
     &                            ientrytab)
               k(NN)     = hconduc(NN,h(NN),theta(NN),cofgen,swfrost,   &
     &                          rfcp(NN),swsophy,numtab,sptab,ientrytab)
!              in case of static macropores FrArMtrx < 1
               if(swmacro.eq.1)  k(NN) = FrArMtrx(NN) * k(NN)
               kmean(NN+1) = hcomean(swkmean,k(NN),cofgen(3,(NN+1))     &
     &                       ,dz(NN),dz(NN+1))
...
!                 pressure head at lower boundary specified  
                  ...
                  kmean(numnod+1) = hconduc(numnod,h(numnod),           &
     &                   theta(numnod),cofgen,swfrost,rfcp(numnod),     &
     &                            swsophy,numtab,sptab,ientrytab)
                  if(swmacro.eq.1) then
                     kmean(numnod+1) = FrArMtrx(numnod)*kmean(numnod+1)
                  endif
...
!            if(numbit.gt.1)then
!            Write(500,' (a,f14.6,a,1pe14.6,3(a,i4),a,1pe14.6) ')        &
!     &        't1900: ',t1900,' dt: ',dt, ' numbit: ',numbit,' itry: ', &
!     &        itry, ' numnodFmax: ',numnodFmax, 'Fmax: ',Fmax
!         
!            end if
