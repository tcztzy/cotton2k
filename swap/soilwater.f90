! File VersionID:
!   $Id: soilwater.f90 298 2016-07-25 20:07:31Z kroes006 $
! ----------------------------------------------------------------------
...
! --- in case of preferential flow, adjust Van Genuchten parameters
      do i = 1, numnod
        theta(i) = watcon(i,h(i),cofgen,swsophy,numtab,sptab,ientrytab)
      end do

! --- hydr. conductivities, differential moisture capacities
!     and mean hydraulic conductivities for each node
      do node = 1,numnod
        dimoca(node) = moiscap(node,h(node),cofgen,dt,                  &
     &                                  swsophy,numtab,sptab,ientrytab)

        FrArMtrx(node) = 1.d0
        k(node) = hconduc (node,h(node),theta(node),cofgen,swfrost,     &
     &                     rfcp(node),swsophy,numtab,sptab,ientrytab)
...
! --- update parameters for soil water hystereses
     ...
     &            dimoca,theta,disnod,dt,swsophy,numtab,sptab,ientrytab)
