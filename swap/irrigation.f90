! File VersionID:
!   $Id: irrigation.f90 298 2016-07-25 20:07:31Z kroes006 $
! ----------------------------------------------------------------------
...
! -5b - in case of solutes: allow overirrigation when conc exceeds concthreshold
          swcirrthres = 0
          cirrthres = 0.0d0
          perirrsurp = 0.0d0
...
!     scheduled timing within desired period ?
     ...
     &   flCropCalendar .and. .not. flCropHarvest .and. flIrriTime) then
        cirr = cirrs
        isua = isuas
...
!        phlo = -100.0d0 ! field capacity became input (from swap32(6)
        ...
        cdef = 0.0d0
        do node = 1,noddrz
          wclo = watcon(node,phlo,cofgen,swsophy,numtab,sptab,ientrytab)&
     &                                                        * dz(node)
          if (node.eq.noddrz) wclo = wclo*frlow
          wcme = watcon(node,phme,cofgen,swsophy,numtab,sptab,ientrytab)&
     &                                                        * dz(node)
          if (node.eq.noddrz) wcme = wcme*frlow
          wchi = watcon(node,phhi,cofgen,swsophy,numtab,sptab,ientrytab)&
     &                                                        * dz(node)
          if (node.eq.noddrz) wchi = wchi*frlow
          wcac = watcon(node,h(node),cofgen,swsophy,                    &
     &                                numtab,sptab,ientrytab) * dz(node)
