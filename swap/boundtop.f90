! File VersionID:
!   $Id: boundtop.f90 311 2016-12-22 12:17:11Z kroes006 $
! ----------------------------------------------------------------------
      subroutine boundtop 
! ----------------------------------------------------------------------
...
! --- Calculate hydraulic conductivity corresponding with hAtm
      if (hAtm.lt.0.0d0) Then
         TheAtm = watcon(1,dble(hatm),cofgen,swsophy,numtab,sptab,      &
     &                   ientrytab)
         ksurf  = hconduc (1,dble(hatm),TheAtm,cofgen,swfrost,rfcp(1),  &
